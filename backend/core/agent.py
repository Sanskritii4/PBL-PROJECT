...
class QLearningAgent:
    ...
    def train_with_callback(
        ...
        The callback is called every `report_every` episodes with a dict:
            {"episode", "total_episodes", "avg_reward", "delivery_rate", "epsilon", "q_table_size"}
        """
        nodes = list(self.network.graph.nodes())
        ...
        logger.info("Training complete (callback mode). Q-table has %d entries.", len(self.q_table))
        return self.training_history
