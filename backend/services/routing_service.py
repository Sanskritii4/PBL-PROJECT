...
class RoutingService:
    ...
    def train_agent_streaming(self, episodes: int = 5000, alpha: float = 0.1,
                               gamma: float = 0.95, epsilon: float = 1.0,
                               epsilon_min: float = 0.01, epsilon_decay: float = 0.995,
                               fluctuate_every: int = 50, report_every: int = 50,
                               on_progress=None) -> dict:
        """Train with progress callback for WebSocket streaming."""
        hp = HyperParameters(
        ...
        t0 = time.time()
        history = self.agent.train_with_callback(
            episodes=episodes,
    ...
    def compare_routes(self, num_packets: int = 50, seed: Optional[int] = None) -> dict:
        ...
        ql_all_paths, dj_all_paths = [], []

        for src, dst in pairs:
            ql = self.route_q_learning(src, dst)
            dj = self.route_dijkstra(src, dst)

            ql_all_paths.append(ql["path"])
            dj_all_paths.append(dj["path"])
        ...
        return {
            ...
            "ql_paths": ql_all_paths,
            "dj_paths": dj_all_paths,
        }
